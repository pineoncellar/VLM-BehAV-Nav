...
class LocalRRTStar:
    ...
    def edge_valid(self, n1: RRTStarNode2D, n2: RRTStarNode2D, samples: int = 14) -> bool:
    ...
    def edge_semantic_cost(self, n1: RRTStarNode2D, n2: RRTStarNode2D, samples: int = 14) -> float:
        vals = []
        for t in np.linspace(0.0, 1.0, samples):
            x = n1.x + t * (n2.x - n1.x)
            y = n1.y + t * (n2.y - n1.y)
...
class FtFootStyleRGBDRRTStar(Node):
    ...
    def smooth_cost_map(self) -> None:
        ...
        src = self.cost_map.astype(np.float32)
        mask = self.cost_map_valid.astype(np.float32)
