...
class MPCConfig:
    ...
    # ---------- robot / planner ----------
    goal_reach_threshold: float = 0.20
    ...
    semantic_conf_threshold: float = 0.20
    grass_dilate_threshold: float = 0.25
    grass_dilate_kernel: int = 11
    ...
    # ---------- anchor / semantic grid ----------
    obstacle_anchor_count: int = 12
    semantic_anchor_count: int = 42
    sigma_obs: float = 0.35
    sigma_sem: float = 0.60
    ...
    # ---------- cost weights ----------
    q_goal_stage: float = 0.12
    q_goal_terminal: float = 10.0
    q_goal_terminal_yaw: float = 1.0

    q_sem: float = 8.0
    q_obs: float = 8.0
    q_progress: float = 0.5
...
class AcadosBehaviorMPC:
    ...
    改动要点：
    1. 参数里加入当前机器人位姿 [robot_x, robot_y, robot_yaw]
    2. 障碍物仍在 odom 下建模
    3. 语义不再用 sem_x / sem_y / sem_w 三元组离散高斯乱加
    4. 改为：
       - 固定 robot frame 语义网格点
       - 每个点只传入一个 runtime score
       - MPC 内部把预测点转回 robot frame
       - 用归一化 RBF 构造连续语义场
    """
    ...
    def __init__(self, cfg: MPCConfig):
        ...
        self.sem_grid_xy = [
            (float(xr), float(yr))
            for xr in cfg.semantic_x_samples
            for yr in cfg.semantic_y_samples
        ]
        self.n_sem = len(self.sem_grid_xy)

        # p = [goal(3), robot_pose(3), obs_x(n_obs), obs_y(n_obs), obs_w(n_obs), sem_vals(n_sem)]
        self.np_dim = 6 + 3 * self.n_obs + self.n_sem
    ...
    def _build_model(self) -> AcadosModel:
        ...
        sem_vals = p[sem_start: sem_start + self.n_sem]
        ...
        goal_dir = ca.atan2(goal_y - py, goal_x - px)
        progress_reward = self.cfg.q_progress * v * ca.cos(yaw - goal_dir)
        ...
        # ---------- obstacle field in odom ----------
        obs_cost = 0
        for i in range(self.n_obs):
            d2 = (px - obs_x[i]) ** 2 + (py - obs_y[i]) ** 2
            obs_cost += obs_w[i] * ca.exp(-d2 / (2.0 * self.cfg.sigma_obs ** 2))

        # ---------- semantic field in robot frame ----------
        dx_r = px - robot_x_odom
        dy_r = py - robot_y_odom

        x_rob = dx_r * ca.cos(robot_yaw_odom) + dy_r * ca.sin(robot_yaw_odom)
        y_rob = -dx_r * ca.sin(robot_yaw_odom) + dy_r * ca.cos(robot_yaw_odom)

        sem_num = 0
        sem_den = 1e-6

        for i, (gx, gy) in enumerate(self.sem_grid_xy):
            d2 = (x_rob - gx) ** 2 + (y_rob - gy) ** 2
            basis = ca.exp(-d2 / (2.0 * self.cfg.sigma_sem ** 2))
            sem_num += sem_vals[i] * basis
            sem_den += basis

        # 归一化后的连续语义场，范围更稳定
        sem_field = sem_num / sem_den

        stage_cost = (
            goal_stage_cost
            + regularization_cost
            + self.cfg.q_obs * obs_cost
            + self.cfg.q_sem * sem_field
            - progress_reward
        )
    ...
    def solve(self, x0: np.ndarray, p_vec: np.ndarray) -> Tuple[float, float, int]:
        ...
        v_cmd = float(np.clip(x1[3], self.cfg.v_min, self.cfg.v_max))
        w_cmd = float(np.clip(x1[4], self.cfg.w_min, self.cfg.w_max))
...
class PerceptionAnchorBuilder:
    ...
    改动要点：
    1. obstacle 仍然是 [x, y, weight]
    2. semantic 不再是 [x, y, weight] 三元组
    3. semantic 改成固定 robot-frame 网格上的一维 score 数组
    """
    ...
    def __init__(self, cfg: MPCConfig, geom: GeometryProjectionUtils):
        ...
        self.n_sem = len(self._semantic_robot_grid)
        self.semantic_grid_vals = np.zeros((self.n_sem,), dtype=np.float64)

    def reset(self) -> None:
        self.obstacle_anchors[:] = 0.0
        self.semantic_grid_vals[:] = 0.0
    ...
    def update_obstacle_anchors(
        ...
        # 归一化到 [0,1]，避免障碍代价动不动淹没目标项
        raw = 1.0 / (clearance ** 2)
        weights = raw / (raw + 10.0)

        near_mask = clearance < self.cfg.safe_dist_threshold
        weights[near_mask] *= 1.2
    ...
    def _compute_semantic_score_from_maps(
        ...
        probs = np.stack([veg, pave, grass, stop], axis=0)   # shape (4, N)
        cls_idx = np.argmax(probs, axis=0)
        cls_conf = np.max(probs, axis=0)

        score = np.zeros_like(cls_conf, dtype=np.float64)
        valid = cls_conf >= self.cfg.semantic_conf_threshold

        veg_mask = valid & (cls_idx == 0)
        pave_mask = valid & (cls_idx == 1)
        grass_mask = valid & (cls_idx == 2)
        stop_mask = valid & (cls_idx == 3)

        score[veg_mask] = self.cfg.semantic_class_weights["vegetation"] * cls_conf[veg_mask]
        score[pave_mask] = self.cfg.semantic_class_weights["Pavement"] * cls_conf[pave_mask]
        score[grass_mask] = self.cfg.semantic_class_weights["grass"] * cls_conf[grass_mask]
        score[stop_mask] = self.cfg.semantic_class_weights["Stop gesture"] * cls_conf[stop_mask]
    ...
    def update_semantic_anchors(
        ...
        if semantic_maps is None or len(semantic_maps) == 0:
            return self.semantic_grid_vals
        ...
        self.semantic_grid_vals[valid_indices] = np.clip(sem_score, 0.0, 1.0)
        return self.semantic_grid_vals
    ...
    def build_parameter_vector(
        ...
        # p = [goal(3), robot_pose(3), obs_x, obs_y, obs_w, sem_vals]
        p_dim = 6 + 3 * n_obs + n_sem
        ...
        p[sem_base: sem_base + n_sem] = self.semantic_grid_vals
...
class BehaviorMPCPlannerNode(Node):
    ...
    def image_callback(self, msg: Image) -> None:
        ...
        try:
            ...
            # 对 grass 做膨胀，让避草更明显
            grass_prob = self.semantic_maps["grass"]
            grass_mask = (grass_prob > self.cfg.grass_dilate_threshold).astype(np.uint8) * 255
            kernel = np.ones((self.cfg.grass_dilate_kernel, self.cfg.grass_dilate_kernel), np.uint8)
            grass_mask_dilated = cv2.dilate(grass_mask, kernel, iterations=1)
            grass_dilated = grass_mask_dilated.astype(np.float32) / 255.0
            self.semantic_maps["grass"] = np.maximum(self.semantic_maps["grass"], grass_dilated)
    ...
    def build_mpc_parameter_vector(self) -> np.ndarray:
        ...
        return self.anchor_builder.build_parameter_vector(
            ...
            robot_x=float(self.x),
            robot_y=float(self.y),
            robot_yaw=float(self.th),
        )
    ...
    def solve_current_mpc(self) -> Tuple[float, float, int]:
        ...
        # 每次求解前，用当前姿态刷新一次语义网格
        if (
            len(self.semantic_maps) > 0
            and self.img_h is not None
            and self.img_w is not None
            and self.x is not None
            and self.y is not None
            and self.th is not None
        ):
            self.anchor_builder.update_semantic_anchors(
                semantic_maps=self.semantic_maps,
                img_h=self.img_h,
