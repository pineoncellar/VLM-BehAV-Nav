...
class AcadosBehaviorMPC:
    ...
    def _build_model(self) -> AcadosModel:
        ...
        sem_cost = sem_cost / self.n_sem

        
        stage_cost = goal_stage_cost + regularization_cost + obs_cost + self.cfg.q_sem * sem_cost - progress_reward
...
class PerceptionAnchorBuilder:
    ...
    def update_semantic_anchors(
        ...
        # 不再取 top-k，而是保留整片局部区域的语义采样
        sem_score = np.clip(sem_score, 0.0, 1.0)
        n = min(len(pts_odom_valid), self.cfg.semantic_anchor_count)

        self.semantic_anchors[:] = 0.0
        self.semantic_anchors[:n, 0:2] = pts_odom_valid[:n]
        self.semantic_anchors[:n, 2] = sem_score[:n]
