...
class LandmarkDetectorNode(Node):
    ...
    def load_landmarks_from_file(self, file_path):
        ...
        try:
            self.get_logger().info(f"Loading landmarks from {file_path}")  # 打印日志
            with open(file_path, "r") as f:
                data = json.load(f)
                landmarks = data.get("landmarks", [])
                self.get_logger().info(f"Loaded landmarks: {landmarks}")  # 打印读取的地标
                return landmarks
        except Exception as e:
            self.get_logger().error(f"读取地标文件失败: {e}")
            return []  # 如果读取失败，返回空列表
    ...
    def image_callback(self, msg: Image):
        self.get_logger().info('Received an image message')  # 打印日志
        try:
            cv_image = self.bridge.imgmsg_to_cv2(msg, desired_encoding='passthrough')
            self.get_logger().info(f"Image converted successfully, shape: {cv_image.shape}")  # 打印图像形状
    ...
    def timer_callback(self):
        if self.latest_image is None or self.is_processing:
            self.get_logger().info("No image or already processing...")  # 调试日志
            return

        self.get_logger().info("Starting image processing...")  # 调试日志
        self.is_processing = True
        try:
            self.process_image(self.latest_image.copy())  # 处理图像
        except Exception as e:
    ...
    def process_image(self, image_bgr):
        self.get_logger().info("Processing image...")  # 打印日志
        ...
        if not response_text:
            self.get_logger().error(f"Failed to get bounding box and distance for {target_text}")  # 错误日志
            return

        parsed = self.parse_vlm_response(response_text)
        if not parsed.get("visible"):
            self.get_logger().error(f"Target {target_text} not visible or invalid response")
            return

        self.get_logger().info(f"Target bbox: {parsed['x_min']}, {parsed['y_min']}, {parsed['x_max']}, {parsed['y_max']}")  # 打印bounding box
...
def main(args=None):
    ...
    try:
        rclpy.spin(node)  # 启动 ROS 节点
    except KeyboardInterrupt:
        pass  # 捕获键盘中断并退出
    finally:
        node.destroy_node()  # 销毁节点
        rclpy.shutdown()  # 确保只调用一次 shutdown()
